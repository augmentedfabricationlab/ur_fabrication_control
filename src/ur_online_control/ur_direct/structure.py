...
class URCommandScript(AirpickMixins):
    ...
    def add_move_joint(self, joint_values, feedback=None):
        """Add a move command to the script"""
        #base, shoulder, elbow, wrist1, wrist2, wrist3
        move = zip(range(len(joint_values)), joint_values)
        [j0, j1, j2, j3, j4, j5, v] = move
        self.add_line("\tmovej({}, {}, {}, {}, {}, {}, v={})".format(j0, j1, j2, j3, j4, j5, v/10))
        if feedback == "Full":
            self.get_current_pose_joints(True)
        elif feedback == "UR_only":
            self.get_current_pose_joints(False)
        else:
            pass

    def add_move_joints(self, joint_values_list=[], feedback=None):
        for joint_vals in joint_values_list:
            self.add_move_joint(joint_vals)
